...
#include <unistd.h>
#include <time.h>
#include <errno.h> //para detectar o timeout
...
void moveLogs(const char *origem, const char *destino, const char *palavra, ThreadData *t)
{
    ...
    ts.tv_sec += 2;  //tempo limete para detectar deadlock
    ...
    int lockorig = pthread_mutex_timedlock(&arquivos[idOrigem].mutex, &ts);
    ...
    t->acessando[idOrigem] = false;
    //pthread_mutex_unlock(&arquivos[idOrigem].mutex); para o deadlock ocorrer de fato
